...
When online boards are enabled, boards are saved in the MikroCanvas API SQLite database instead. The browser keeps only a small local list of board IDs it has opened so the board library can show recently used online boards without exposing a public board directory.
