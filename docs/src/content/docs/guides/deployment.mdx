...
The server stores boards in SQLite and serves `config.json` with API mode enabled. A board URL includes `?board=<board-id>`, and anyone with that ID can open, update, or delete the board.

Use `mikrocanvas.config.json`, environment variables, or CLI flags to set the host, port, app URL, and database path. See `mikrocanvas.config.json.example` for the file shape.
